...
class StateTemplate:
	def __init__(self, state_name: str, start_function, stop_function, start_message: str, stop_message: str):
		...
		if not start_message:
			self.start_message = f'{self.name} started' # VAL INFO - Start Message
		else:
			self.start_message = start_message


		if not stop_message:
			self.stop_message = f'{self.name} stopped' # VAL INFO - Stop Message
		else:
			self.stop_message = stop_message
			

		if isinstance(start_function, types.FunctionType):
			self.start_function = start_function # VAL INFO - Start Function
		else:
			self.start_function = lambda: 0


		if isinstance(stop_function, types.FunctionType):
			self.stop_function = stop_function # VAL INFO - Stop Function
		else:
